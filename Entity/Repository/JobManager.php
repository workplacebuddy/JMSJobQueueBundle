...
class JobManager
{
    ...
    public function __construct(EntityManagerInterface $em, EventDispatcherInterface $eventDispatcher, RetryScheduler $retryScheduler)
    {
        $this->em = $em;
    ...
    {
        return $this->em->createQuery("SELECT j FROM JMS\\JobQueueBundle\\Entity\\Job j WHERE j.command = :command AND j.args = :args")
    ...
    public function getOrCreateIfNotExists($command, array $args = array())
    {
        ...
        $firstJob = $this->em->createQuery("SELECT j FROM JMS\\JobQueueBundle\\Entity\\Job j WHERE j.command = :command AND j.args = :args ORDER BY j.id ASC")
             ->setParameter('command', $command)
    ...
    public function findAllForRelatedEntity($relatedEntity)
    {
        ...
        return $this->em->createNativeQuery("SELECT j.* FROM jms_jobs j INNER JOIN jms_job_related_entities r ON r.job_id = j.id WHERE r.related_class = :relClass AND r.related_id = :relId", $rsm)
    ...
    private function getRelatedEntityIdentifier($entity)
    {
        ...
        $relId = $this->em->getMetadataFactory()
                    ->getMetadataFor($entity::class)->getIdentifierValues($entity);
        asort($relId);

        if ( ! $relId) {
            throw new \InvalidArgumentException(sprintf('The identifier for entity of class "%s" was empty.', $entity::class));
        }

        return array($entity::class, json_encode($relId));
    }
        ...
        switch ($finalState) {
            ...
            case Job::STATE_FINISHED:
                if ($job->isRetryJob()) {
                    ...
                    $this->em->persist($job->getOriginalJob());
                }
                $job->setState($finalState);
                $this->em->persist($job);
    ...
    public function findIncomingDependencies(Job $job)
    {
        ...
        return $this->em->createQuery("SELECT j, d FROM JMS\\JobQueueBundle\\Entity\\Job j LEFT JOIN j.dependencies d WHERE j.id IN (:ids)")
    ...
    public function getIncomingDependencies(Job $job)
    {
        ...
        return $this->em->createQuery("SELECT j FROM JMS\\JobQueueBundle\\Entity\\Job j WHERE j.id IN (:ids)")
                    ->setParameter('ids', $jobIds)
                    ->getResult();
    }
    ...
    {
        return $this->em->createQuery("SELECT j FROM JMS\\JobQueueBundle\\Entity\\Job j WHERE j.state IN (:errorStates) AND j.originalJob IS NULL ORDER BY j.closedAt DESC")
    ...
    {
        $queues =  $this->em->createQuery("SELECT DISTINCT j.queue FROM JMS\\JobQueueBundle\\Entity\\Job j WHERE j.state IN (:availableStates)  GROUP BY j.queue")
    ...
    {
        $result = $this->em->createQuery("SELECT j.queue FROM JMS\\JobQueueBundle\\Entity\\Job j WHERE j.state IN (:availableStates) AND j.queue = :queue")
            ->setParameter('availableStates', array(Job::STATE_RUNNING, Job::STATE_NEW, Job::STATE_PENDING))
