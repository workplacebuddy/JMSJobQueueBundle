...
class CleanUpCommand extends Command
{
    ...
    private function findStaleJobs()
    {
        ...
        do {
            ...
            /** @var Job $job */
            $job = $this->em->createQuery("SELECT j FROM JMS\\JobQueueBundle\\Entity\\Job j
    ...
    private function cleanUpExpiredJobs(InputInterface $input)
    {
        ...
        foreach ($this->findExpiredJobs($input) as $job) {
            ...
            if ($result->fetchOne() !== false) {
                $this->em->wrapInTransaction(function() use ($job) {
    ...
    private function findExpiredJobs(InputInterface $input)
    {
        $succeededJobs = function(array $excludedIds) use ($input) {
            return $this->em->createQuery("SELECT j FROM JMS\\JobQueueBundle\\Entity\\Job j WHERE j.closedAt < :maxRetentionTime AND j.originalJob IS NULL AND j.state = :succeeded AND j.id NOT IN (:excludedIds)")
        ...
        $finishedJobs = function(array $excludedIds) use ($input) {
            return $this->em->createQuery("SELECT j FROM JMS\\JobQueueBundle\\Entity\\Job j WHERE j.closedAt < :maxRetentionTime AND j.originalJob IS NULL AND j.id NOT IN (:excludedIds)")
        ...
        $canceledJobs = function(array $excludedIds) use ($input) {
            return $this->em->createQuery("SELECT j FROM JMS\\JobQueueBundle\\Entity\\Job j WHERE j.state = :canceled AND j.createdAt < :maxRetentionTime AND j.originalJob IS NULL AND j.id NOT IN (:excludedIds)")
                ->setParameter('maxRetentionTime', new \DateTime('-'.$input->getOption('max-retention')))
